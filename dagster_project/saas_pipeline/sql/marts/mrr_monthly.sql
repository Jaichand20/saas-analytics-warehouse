-- Company-wide MRR trend: builds a full account x month grid, forward-fills
-- each account's MRR from its last subscription event (LAST_VALUE ... IGNORE
-- NULLS - MRR doesn't change between events, so a month with no event for an
-- account still has whatever MRR it was last set to), then LAG for MoM growth.
CREATE OR REPLACE TABLE marts.mrr_monthly AS
WITH months AS (
...
company_month_mrr AS (
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN account_mrr > 0 THEN account_id END) AS active_accounts
